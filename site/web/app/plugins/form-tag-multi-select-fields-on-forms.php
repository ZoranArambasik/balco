...
function wpcf7_selct_multiselct_formtag_handler( $tag ){
    ...
    if ( $tag->has_option( 'placeholder' ) or 
      ...
      $atts['data-allow-clear'] = 'true';
        ...
        if (strpos($atts['class'], 'menu-storlekar')) {
            $atts['data-placeholder'] = __('ÖNSKAD LÄGENHETSSTORLEK*', 'yb-sml');
            $atts['data-allow-clear'] = 'true';
        } 
        elseif (strpos($atts['class'], 'menu-vaning')) {
            $atts['data-placeholder'] = __('ÖNSKAD VÅNING*', 'yb-sml');
            $atts['data-allow-clear'] = 'true'; 
        } else {
            $atts['data-placeholder'] = __('VÄNLIGEN VÄLJ', 'yb-sml');
            $atts['data-allow-clear'] = 'true'; 
        }
